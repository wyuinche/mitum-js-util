...
class BaseKey {
    ...
    constructor(key, type) {
        if (type == 'unknown') {
            ...
            let idx = parsed.hint.indexOf("-" + VERSION);
            this.hintNoVersion = this.hint.substring(0, idx);
     ...
     */
    hinted() {
        return this.key + Xhint.HINT_DIVIDER + this.hint;
    }

    hintedNoVersion() {
        return this.key + Xhint.HINT_DIVIDER + this.hintNoVersion;
    }
     ...
     */
    buffer() {
        return Buffer.from(this.hintedNoVersion());
    }
...
class Keys {
    ...
    get address() {
        return bs58.encode(this.hash) + Xhint.HINT_DIVIDER + Xhint.MC_ADDRESS + '-' + VERSION;
    }
...
class Keypair {
     ...
     */
    getPublicKey() {
        return this.pubKey + Xhint.HINT_DIVIDER + Xutil._hint(this.type);
    }
