...
class Key {
    ...
    dict() {
        const key = {};
        key._hint = this.hint;
        key.weight = this.weight.value.toString();
...
class Keys {
    ...
    dict() {
        ...
        keys.hash = bs58.encode(this.hash.digest());
        ...
        keys.keys = _keys;
        keys.threshold = this.threshold.value.toString();
...
class BTCKeypair extends Keypair {
    ...
    sign(msg) {
        const _msg = Xutil.sha256(Xutil.sha256(msg).digest()).digest();
...
class ETHKeypair extends Keypair {
    ...
    sign(msg) {
        const _msg = Xutil.sha256(msg).digest();
        const sk = this.keypair.getPrivate('hex');
