...
class BaseKey {
    constructor(key, type){
        if(type == 'unknown'){
            const parsed = util.parseHinted(key);
...
class Key {
    constructor(key, w){
        ...
        this.weight = new util.BigInteger(w);
    }
...
class Keys {
    constructor(keys, threshold){
        ...
        this.keys = keys;
        this.threshold = new util.BigInteger(threshold);
...
class Keypair {
    constructor(privKey, type) {
        switch(type) {
            case hint.BTC_PBLCKEY:
                this.privKey = new BaseKey(privKey, hint.BTC_PRIVKEY);
                break;
            case hint.ETHER_PBLCKEY:
                this.privKey = new BaseKey(privKey, hint.ETHER_PRIVKEY);
                break;
            case hint.STELLAR_PBLCKEY:
                this.privKey = new BaseKey(privKey, hint.STELLAR_PRIVKEY);
                break;
            default:
    ...
    getPublicKey() {
        return this.pubKey + '-' + util._hint(this.type);
    }
...
class BTCKeypair extends Keypair {
    constructor(privKey) {
        super(privKey, hint.BTC_PBLCKEY);
        this._generatePublicKey();
    }

    sign(msg) {
        const _msg = util.sha256(util.sha256(msg).digest()).digest();
...
class ETHKeypair extends Keypair {
    constructor(privKey) {
        super(privKey, hint.ETHER_PBLCKEY);
        this._generatePublicKey();
    }

    sign(msg) {
        const _msg = util.sha256(msg).digest();
        const sk = this.keypair.getPrivate('hex');
...
class StellarKeypair extends Keypair {
    constructor(privKey) {
        super(privKey, hint.STELLAR_PBLCKEY);
        this._generatePublicKey();
    }
