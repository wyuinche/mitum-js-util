...
const { HINT_DIVIDER, MC_ADDRESS, PRIVATE_KEY, PUBLIC_KEY } = require('./hint');
...
/**
 * @function
 * Parse hinted strings.
 * @param {string} hinted - Hinted string
 * @return {object} - Parsed result # object{hintless, hint}
 */
const parseHinted = (hinted) => {
    const idx = hinted.indexOf(HINT_DIVIDER);
    const parsed = {};
...
/**
 * @function
 * Parse typed address and typed key.
 * @param {string} hinted - Typed address/key
 * @return {object} - Parsed result # object{typeless, type}
 */
const parseTyped = (typed) => {
    const type = typed.substring(typed.length - 3);
    const typeless = typed.substring(0, typed.length - 3);
