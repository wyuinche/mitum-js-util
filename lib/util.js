...
class BigInteger {
    ...
    constructor(big) {
        ...
        if (big.length >= 16) {  
            ...
            const tmp = bigInt(big);
            this.up = tmp.divide(Math.pow(256, 6));
            this.down = tmp.mod(Math.pow(256, 6));
        }
        else {
            this.big = bigInt(big);
        }
    ...
    get value() {
        if (this.big) {
            ...
            return this.up.multiply(Math.pow(256, 6)).add(this.down).valueOf();
        }
    }
    ...
    tight() {
        ...
        if (this.big) {
            ...
            var up = new BigInteger(this.up.toString());
            var down = new BigInteger(this.down.toString());

            const downTight = down.tight();

            const _buf = Buffer.alloc(6);
            downTight.copy(_buf, 6 - downTight.length, 0, downTight.length);

            return Buffer.concat([up.tight(), _buf]);
        }
    }
