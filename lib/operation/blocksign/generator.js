...
const { MBS_CREATE_DOCUMENTS_SINGLE_FILE, MBS_SIGN_ITEM_SINGLE_DOCUMENT, 
        MBS_CREATE_DOCUMENTS_OP_FACT, MBS_SIGN_DOCUMENTS_OP_FACT } = require('../../hint');
...
class BlockSignGenerator extends OperationGenerator {
    ...
    createCreateDocumentsItem(fileHash, did, signcode, title, size, cid, signers, signcodes) {
        return this.getCreateDocumentsItem(fileHash, did, signcode, title, size, cid, signers, signcodes);
    }
     ...
     * @returns {CreateDocumentsItem} CreateDocumentsItem
     */
    getCreateDocumentsItem(fileHash, did, signcode, title, size, cid, signers, signcodes) {
    ...
    createSignDocumentsItem(owner, did, cid) {
        return this.getSignDocumentsItem(owner, did, cid);
    }
     ...
     * @returns {SignDocumentsItem} SignDocumentsItem
     */
    getSignDocumentsItem(owner, did, cid) {
     ...
     * @deprecated
     * @param {string} type Operation type - [ BLOCKSIGN_CREATE_DOCUMENTS | BLOCKSIGN_SIGN_DOCUMENTS ] which are constant variable of Generator
     * @param {string} sender Sender address with type suffix (mca)
     * @param {BlockSignItem} items BlockSignItem - [CreateDocumentsItem | SignDocumentsItem ] generated by Generator
     * @returns {BlockSignFact} BlockSignFact
     */
    createBlockSignFact(type, sender, items) {
        return this.getBlockSignFact(type, sender, items);
    }
     ...
     * @returns {BlockSignFact} BlockSignFact
     */
    getBlockSignFact(type, sender, items) {
        let _type;
        switch (type) {
