...
const { 
    ...
    MNFT_APPROVE_OP, MNFT_DELEGATE_OP_FACT, MNFT_DELEGATE_OP, MNFT_SIGN_OP_FACT, MNFT_SIGN_OP, MNFT_COLLECTION_POLICY_UPDATER_OP_FACT, MNFT_COLLECTION_POLICY_UPDATER_OP 
} = require("../../hint");
...
class CollectionPolicyUpdaterFact extends PurposedOperationFact {
    constructor(sender, collection, policy, cid) {
        super(MNFT_COLLECTION_POLICY_UPDATER_OP_FACT);
        this.sender = new Address(sender);
    ...
    buffer() {
        ...
        const bPolicy = this.policy.buffer();
        const bCid = Buffer.from(this.cid);

        return Buffer.concat([bToken, bSender, bCollection, bPolicy, bCid]);
    }
    ...
    dict() {
        ...
        fact.token = Buffer.from(this.token.toISOString(), 'utf8').toString('base64');
        fact.sender = this.sender.typed();
        fact.collection = this.collection;
        fact.policy = this.policy.dict();
        fact.currency = this.cid;
    ...
    get operationHint() {
        return MNFT_COLLECTION_POLICY_UPDATER_OP;
    }
}
