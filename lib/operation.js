...
class Address {
     ...
     */
    constructor(addr) {
        let parsed = Xutil.parseTyped(addr);

        this.type = parsed.type;
        this.addr = parsed.typeless;

        if(this.type !== Xhint.MC_ADDRESS) {
    ...
    /**
     * @method
     * Returns Buffer of typeless address.
     * @return {Buffer} - Buffered typeless address 
     */
    buffer() {
...
class CreateDocumentsItem extends BlockSignItem {
    constructor(type, fileHash, did, signcode, title, size, cid, signers, signcodes) {
        ...
        this.title = title;
        this.size = new Xutil.BigInteger(size.toString());
        this.cid = cid;
